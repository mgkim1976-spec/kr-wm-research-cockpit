...
def _llm_summarize(name: str, ticker: str, house: list, others: list) -> dict:
    ...
    prompt = (
        ...
        "  - 카탈리스트: 본문이 언급한 **향후 예정 이벤트**를 적극적으로 찾아 적어라 — 분기 실적 발표, 신규 수주·계약, "
        "신제품·신차 출시, 정책·규제, IR/학회, 가이던스 변경 등. 해당되면 시점(분기·반기·날짜)과 함께 1문장. "
        "본문에 그런 forward-looking 이벤트가 전혀 없을 때만 빈 문자열(\"\"). (없는 일정 창작만 금지)\n"
