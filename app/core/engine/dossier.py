...
def _llm_summarize(name: str, ticker: str, house: list, others: list) -> dict:
    ...
    prompt = (
        ...
        "  - 카탈리스트: 본문이 언급한 **오늘 날짜 이후의 향후 예정 이벤트**를 적극적으로 찾아 적어라 — 다음 분기 실적 발표, "
        "신규 수주·계약, 신제품·신차 출시, 정책·규제, IR/학회, 가이던스 변경 등. 해당되면 시점(분기·반기·날짜)과 함께 1문장. "
        "**이미 지난 실적·이벤트(오늘 이전)는 카탈리스트가 아니다 — 제외.** "
        "오늘 이후 forward-looking 이벤트가 전혀 없으면 빈 문자열(\"\"). (없는 일정 창작만 금지)\n"
        ...
        '"catalysts":"본문이 언급한 향후 예정 이벤트(실적발표·수주·신제품·정책 등)를 시점과 함께. 전혀 없으면 빈 문자열"}'
    )
    try:
